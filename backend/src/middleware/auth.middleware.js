...
function jwtRemainingSeconds(token) {
  try {
 ...
 */
export const generateTokens = async (user, meta = {}) => {
  ...
  try {
    ...
    if (!token) {
      ...
      if (!authHeader?.startsWith('Bearer ')) {
        return res.status(401).json({ success: false, error: 'Token tidak ditemukan', code: 'NO_TOKEN' });
      }
    ...
    if (await isBlacklisted(token)) {
      return res.status(401).json({ success: false, error: 'Token sudah dicabut', code: 'TOKEN_REVOKED' });
    }
    ...
    if (!user || !user.is_active) {
      return res.status(401).json({ success: false, error: 'Akun tidak ditemukan atau tidak aktif', code: 'USER_NOT_FOUND' });
    }
    ...
    if (err.name === 'TokenExpiredError') {
      return res.status(401).json({ success: false, error: 'Token sudah kadaluarsa', code: 'TOKEN_EXPIRED' });
    }
    if (err.name === 'JsonWebTokenError') {
      return res.status(401).json({ success: false, error: 'Token tidak valid', code: 'INVALID_TOKEN' });
    }
 ...
 */
export const revokeTokens = async (accessToken, rawRefreshToken) => {
