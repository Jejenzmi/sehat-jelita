...
// ============================================================
async function autoFlag(testCode: string, resultValue: string, patient: { gender?: string; birth_date?: Date | string } | null): Promise<string | null> {
...
// ============================================================
async function deltaCheck(orderId: string, testCode: string, currentValue: string): Promise<{ delta_flag: boolean; previous_value: string | null; pct_change?: number }> {
...
// Type definitions
interface LabOrderBody extends z.infer<typeof labOrderSchema> { }
interface LabResultBody extends z.infer<typeof labResultSchema> { }
 ...
 * Get all lab orders
 */
router.get('/orders', asyncHandler(async (req: Request<Record<string, string>, any, any, LabOrderQuery>, res: Response) => {
  const {
    page = '1',
    limit = '20',
    cursor,
 ...
 */
router.get('/orders/pending', asyncHandler(async (req: Request, res: Response) => {
  const orders = await prisma.lab_orders.findMany({
    where: {
 ...
 */
router.get('/orders/:id', asyncHandler(async (req: Request<{ id: string }>, res: Response) => {
  const { id } = req.params;
 ...
 */
router.post('/orders', requireRole(['admin', 'dokter', 'perawat', 'laboratorium']), asyncHandler(async (req: Request<Record<string, string>, any, LabOrderBody>, res: Response) => {
 ...
 */
router.put('/orders/:id/status', requireRole(['admin', 'laboratorium']), asyncHandler(async (req: Request<{ id: string }, any, UpdateStatusBody>, res: Response) => {
  const { id } = req.params;
 ...
 */
router.post('/orders/:id/results', requireRole(['admin', 'laboratorium']), asyncHandler(async (req: Request<{ id: string }, any, LabResultBody[]>, res: Response) => {
  const { id } = req.params;
  ...
  const criticalResults: Array<{ test_code: string; test_name: string; value: string; unit?: string }> = [];

  for (const result of results) {
    // Auto-flag if not manually provided
    let flag: string | null = result.flag || null;
 ...
 */
router.post('/orders/:id/verify', requireRole(['admin', 'laboratorium']), asyncHandler(async (req: Request<{ id: string }, any, VerifyBody>, res: Response) => {
  const { id } = req.params;
 ...
 */
router.get('/results', asyncHandler(async (req: Request<Record<string, string>, any, any, LabResultQuery>, res: Response) => {
 ...
 */
router.get('/results/patient/:patientId', asyncHandler(async (req: Request<{ patientId: string }, any, any, PatientResultQuery>, res: Response) => {
 ...
 */
router.get('/reference-ranges', asyncHandler(async (req: Request<Record<string, string>, any, any, ReferenceRangeQuery>, res: Response) => {
 ...
 */
router.put('/reference-ranges/:id', requireRole(['admin', 'laboratorium']), asyncHandler(async (req: Request<{ id: string }>, res: Response) => {
  const { id } = req.params;
 ...
 */
router.get('/stats/workload', asyncHandler(async (req: Request, res: Response) => {
 ...
 */
router.get('/print/:orderId', asyncHandler(async (req: Request<{ orderId: string }>, res: Response) => {
