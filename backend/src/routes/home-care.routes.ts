 ...
 */
router.get('/visits', asyncHandler(async (req: Request<{}, {}, {}, VisitQuery>, res: Response) => {
  ...
  if (date_from || date_to) {
    where.visit_date = {};
    if (date_from) (where.visit_date as Record<string, Date>).gte = new Date(date_from);
    if (date_to) (where.visit_date as Record<string, Date>).lte = new Date(date_to);
  }
 ...
 */
router.get('/visits/:id', asyncHandler(async (req: Request<{ id: string }>, res: Response) => {
  const { id } = req.params;
 ...
 */
router.post('/visits', requireRole(['admin', 'registrasi', 'perawat']), asyncHandler(async (req: Request, res: Response) => {
  ...
  const visit = await prisma.home_care_visits.create({ data: { ...data, visit_number } as any });
 ...
 */
router.put('/visits/:id', requireRole(['admin', 'registrasi', 'perawat', 'dokter']), asyncHandler(async (req: Request<{ id: string }>, res: Response) => {
  const { id } = req.params;
  const data = visitSchema.partial().parse(req.body) as Record<string, unknown>;

  // Set completed_at when status changes to completed
  if (data.status === 'completed' && !data.completed_at) {
    (data as Record<string, unknown>).completed_at = new Date();
  }
 ...
 */
router.get('/next-visit-number', asyncHandler(async (req: Request, res: Response) => {
  const today = new Date();
  const dateStr = today.toISOString().slice(0, 10).replace(/-/g, '');
  const count = await prisma.home_care_visits.count();
