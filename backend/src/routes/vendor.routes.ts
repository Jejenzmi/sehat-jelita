...
router.get('/stats', asyncHandler(async (_req: Request, res: Response) => {
  ...
  }).catch(() => ({ _avg: { rating: null } }) as { _avg: { rating: number | null } });
...
// Vendors CRUD
router.get('/', asyncHandler(async (req: Request<Record<string, string>, any, any, VendorsQuery>, res: Response) => {
...
router.post('/', requireRole(['admin', 'pengadaan']), asyncHandler(async (req: Request, res: Response) => {
  const vendor = await prisma.vendors.create({ data: req.body });
  res.status(201).json({ success: true, data: vendor });
}));

router.put('/:id', requireRole(['admin', 'pengadaan']), asyncHandler(async (req: Request<{ id: string }>, res: Response) => {
...
router.delete('/:id', requireRole(['admin', 'pengadaan']), asyncHandler(async (req: Request<{ id: string }>, res: Response) => {
...
// Contracts
router.get('/contracts/list', asyncHandler(async (req: Request<Record<string, string>, any, any, ContractsQuery>, res: Response) => {
...
router.post('/contracts', requireRole(['admin', 'pengadaan']), asyncHandler(async (req: Request, res: Response) => {
  const contract = await prisma.vendor_contracts.create({ data: req.body });
  res.status(201).json({ success: true, data: contract });
}));

router.put('/contracts/:id', requireRole(['admin', 'pengadaan']), asyncHandler(async (req: Request<{ id: string }>, res: Response) => {
