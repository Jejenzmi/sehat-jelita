...
const adminOnly = [authenticateToken, requireRole(['admin', 'it'])] as const;
...
router.put('/config/:id', ...adminOnly, asyncHandler(async (req: Request<{ id: string }>, res: Response) => {
...
router.get('/devices/:code', asyncHandler(async (req: Request<{ code: string }>, res: Response) => {
...
router.post('/devices', ...adminOnly, asyncHandler(async (req: Request, res: Response) => {
  const device = await prisma.smart_display_devices.create({ data: req.body });
  res.status(201).json({ success: true, data: device });
}));

router.put('/devices/:id', ...adminOnly, asyncHandler(async (req: Request<{ id: string }>, res: Response) => {
...
router.delete('/devices/:id', ...adminOnly, asyncHandler(async (req: Request<{ id: string }>, res: Response) => {
  await prisma.smart_display_devices.delete({ where: { id: req.params.id } });
  res.json({ success: true, data: { id: req.params.id } });
}));

// Media
router.get('/media', asyncHandler(async (req: Request<Record<string, string>, any, any, MediaQuery>, res: Response) => {
...
router.post('/media/upload', ...adminOnly, asyncHandler(async (req: Request, res: Response) => {
  ...
  }).catch(() => ({ _max: { display_order: 0 } }) as { _max: { display_order: number | null } });
  ...
  const media = await prisma.smart_display_media.create({
    data: {
      ...
      display_order: ((maxOrder._max as { display_order: number | null }).display_order ?? 0) + 1,
    },
  });
...
router.delete('/media/:id', ...adminOnly, asyncHandler(async (req: Request<{ id: string }>, res: Response) => {
