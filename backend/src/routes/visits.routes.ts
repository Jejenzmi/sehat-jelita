...
type VisitStatus = 'active' | 'completed' | 'serving' | 'waiting' | 'called' | 'discharged' | 'cancelled';
...
interface VisitBody extends z.infer<typeof visitSchema> { }
 ...
 */
router.get('/', checkMenuAccess('rawat_jalan'), asyncHandler(async (req: Request<Record<string, string>, any, any, VisitQuery>, res: Response) => {
  const {
    page = '1',
    limit = '20',
    cursor,
 ...
 * Get today's visits
 */
router.get('/today', asyncHandler(async (req: Request<Record<string, string>, any, any, { visit_type?: string; department_id?: string }>, res: Response) => {
 ...
 */
router.get('/inpatient', checkMenuAccess('rawat_inap'), asyncHandler(async (req: Request<Record<string, string>, any, any, { department_id?: string }>, res: Response) => {
...
router.get('/:id', asyncHandler(async (req: Request<{ id: string }>, res: Response) => {
  ...
  const visit = await prisma.visits.findUnique({
    ...
    include: {
      ...
      beds: {
        select: {
          id: true, bed_number: true,
          // TODO: Add bed_class field to beds schema
          // bed_class: true,
 ...
 */
router.post('/', requireRole(['admin', 'registrasi']), asyncHandler(async (req: Request<Record<string, string>, any, VisitBody>, res: Response) => {
  ...
  const visit = await prisma.$transaction(async (tx) => {
    ...
    return tx.visits.create({
      data: {
        ...
        ...(data.bed_id && { bed_id: data.bed_id }),
        ...(data.chief_complaint && { chief_complaint: data.chief_complaint }),
        ...(data.payment_type && { payment_type: data.payment_type }),
        ...(data.bpjs_sep_number && { bpjs_sep_number: data.bpjs_sep_number }),
        ...(data.insurance_policy_number && { insurance_policy_number: data.insurance_policy_number }),
  ...
  io?.to('queue').emit('visit:created', {
    ...
    patient_name: (visit as any).patients?.full_name,
    department: (visit as any).departments?.department_name
  });
 ...
 * Update visit
 */
router.put('/:id', requireRole(['admin', 'dokter', 'perawat']), asyncHandler(async (req: Request<{ id: string }, any, Partial<VisitBody>>, res: Response) => {
  const { id } = req.params;
 ...
 */
router.post('/:id/discharge', requireRole(['admin', 'dokter']), asyncHandler(async (req: Request<{ id: string }, any, DischargeBody>, res: Response) => {
  const { id } = req.params;
 ...
 */
router.post('/:id/transfer', requireRole(['admin', 'dokter', 'perawat']), asyncHandler(async (req: Request<{ id: string }, any, TransferBody>, res: Response) => {
  const { id } = req.params;
 ...
 */
router.get('/queue/today', asyncHandler(async (req: Request<Record<string, string>, any, any, { department_id?: string }>, res: Response) => {
  const { department_id } = req.query;
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  const tomorrow = new Date(today);
  ...
  const visits = await prisma.visits.findMany({
    ...
    // TODO: Add queue_number field to visits schema for queue ordering
 ...
 */
router.get('/queue/stats', asyncHandler(async (_req: Request, res: Response) => {
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  const tomorrow = new Date(today);
  tomorrow.setDate(tomorrow.getDate() + 1);
 ...
 */
router.patch('/:id/status', asyncHandler(async (req: Request<{ id: string }, any, StatusUpdateBody>, res: Response) => {
  const { id } = req.params;
