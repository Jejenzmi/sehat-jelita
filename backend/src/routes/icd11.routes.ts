 ...
 */
router.get('/search', searchRateLimiter, asyncHandler(async (req: Request<{}, {}, {}, Icd11SearchQuery>, res: Response) => {
 ...
 */
router.get('/entity/:id(*)', asyncHandler(async (req: Request, res: Response) => {
  await ensureCredentials();
  const entityId = decodeURIComponent(req.params.id);
  const { lang = 'id' } = req.query;
  const data = await icd11.getEntity(entityId, lang as string);
...
router.get('/code/:code(*)', asyncHandler(async (req: Request, res: Response) => {
  ...
  const { lang = 'id' } = req.query;
  const data = await icd11.getCodeInfo(code, lang as string);
  res.json({ success: true, data });
}));
...
router.post('/test-connection',
  requireRole(['admin']),
  asyncHandler(async (req: Request<{}, {}, TestConnectionBody>, res: Response) => {
...
router.post('/save-config',
  requireRole(['admin']),
  asyncHandler(async (req: Request<{}, {}, SaveConfigBody>, res: Response) => {
    const { client_id, client_secret } = req.body;
 ...
 */
router.get('/codes', searchRateLimiter, asyncHandler(async (req: Request<{}, {}, {}, CodesQuery>, res: Response) => {
