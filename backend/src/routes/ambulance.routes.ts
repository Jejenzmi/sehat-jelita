 ...
 */
router.get('/fleet', asyncHandler(async (req: Request<{}, {}, {}, FleetQuery>, res: Response) => {
 ...
 */
router.post('/fleet', requireRole(['admin']), asyncHandler(async (req: Request, res: Response) => {
  const data = fleetSchema.parse(req.body);
  const ambulance = await prisma.ambulance_fleet.create({ data: data as any });
 ...
 */
router.put('/fleet/:id', requireRole(['admin']), asyncHandler(async (req: Request<{ id: string }>, res: Response) => {
  const { id } = req.params;
 ...
 */
router.get('/dispatches', asyncHandler(async (req: Request<{}, {}, {}, DispatchQuery>, res: Response) => {
  ...
  if (date_from || date_to) {
    where.request_time = {};
    if (date_from) (where.request_time as Record<string, Date>).gte = new Date(date_from);
    if (date_to) (where.request_time as Record<string, Date>).lte = new Date(date_to);
  }
 ...
 */
router.post('/dispatches', requireRole(['admin', 'registrasi', 'perawat']), asyncHandler(async (req: Request, res: Response) => {
  ...
  const dispatch = await prisma.ambulance_dispatches.create({ data: { ...data, dispatch_number } as any });
 ...
 */
router.put('/dispatches/:id', requireRole(['admin', 'registrasi', 'perawat']), asyncHandler(async (req: Request<{ id: string }>, res: Response) => {
  const { id } = req.params;
  ...
  if (data.status === 'completed' && (dispatch as Record<string, unknown>).ambulance_id) {
    await prisma.ambulance_fleet.update({
      where: { id: (dispatch as Record<string, unknown>).ambulance_id as string },
 ...
 */
router.get('/next-dispatch-number', asyncHandler(async (req: Request, res: Response) => {
  const today = new Date();
  const dateStr = today.toISOString().slice(0, 10).replace(/-/g, '');
  const count = await prisma.ambulance_dispatches.count();
