...
function normalizeJournal(j) {
  return {
    ...
    lines: j.journal_entry_lines?.map(l => ({
      ...l,
      journal_entry_id: l.entry_id,
      debit_amount: Number(l.debit || 0),
      credit_amount: Number(l.credit || 0),
      account: l.chart_of_accounts ? normalizeAccount(l.chart_of_accounts) : undefined,
    })),
  };
}
...
router.get('/accounts',
  ...
  asyncHandler(async (req, res) => {
    ...
    const where = { is_active: active === 'false' ? false : true };
...
router.post('/accounts',
  ...
  asyncHandler(async (req, res) => {
    ...
    res.status(201).json({ success: true, data: normalizeAccount(account) });
  })
);
...
router.put('/accounts/:id',
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json({ success: true, data: normalizeAccount(account) });
  })
);
  ...
  asyncHandler(async (req, res) => {
    ...
    if (cursor) {
      const entries = await prisma.journal_entries.findMany({
        where: { ...where, id: { lt: cursor } }, // newest-first via lt
        take: take + 1,
...
router.get('/journals/:id',
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json({ success: true, data: normalizeJournal(journal) });
  })
);
...
router.post('/journals',
  ...
  asyncHandler(async (req, res) => {
    ...
    res.status(201).json({ success: true, data: normalizeJournal(entry) });
  })
);
  ...
  asyncHandler(async (req, res) => {
    ...
    const entry = await prisma.$transaction(async (tx) => {
      ...
      for (const line of journal.journal_entry_lines) {
        ...
        if (account.normal_balance === 'CREDIT') balanceChange = -balanceChange;
    ...
    res.json({ success: true, data: normalizeJournal(entry) });
  })
);
...
router.get('/reports/general-ledger',
  ...
  asyncHandler(async (req, res) => {
    ...
    const ledger = accounts.map(acc => {
      ...
      if (acc.normal_balance === 'DEBIT')  closingBalance = openingBalance + totalDebit - totalCredit;
    ...
    }).filter(e => e.transaction_count > 0 || e.opening_balance !== 0);
...
router.get('/reports/trial-balance',
  ...
  asyncHandler(async (req, res) => {
    ...
    const trialBalance = accounts.map(acc => ({
      ...
      debit:  acc.normal_balance === 'DEBIT'  ? Number(acc.current_balance || 0) : 0,
      credit: acc.normal_balance === 'CREDIT' ? Number(acc.current_balance || 0) : 0,
    }));
...
router.get('/reports/income-statement',
  ...
  asyncHandler(async (req, res) => {
    ...
    const calcAmount = (acc, isRevenue) => {
      const lines = journalLines.filter(l => l.account_id === acc.id);
      if (isRevenue) return lines.reduce((s, l) => s + Number(l.credit || 0) - Number(l.debit || 0), 0);
      return lines.reduce((s, l) => s + Number(l.debit || 0) - Number(l.credit || 0), 0);
    };

    const revenueItems = revenueAccounts
      .map(acc => ({ ...normalizeAccount(acc), amount: calcAmount(acc, true) }))
      .filter(i => i.amount !== 0);

    const expenseItems = expenseAccounts
      .map(acc => ({ ...normalizeAccount(acc), amount: calcAmount(acc, false) }))
      .filter(i => i.amount !== 0);
...
router.get('/reports/balance-sheet',
  ...
  asyncHandler(async (req, res) => {
    ...
    const assets      = accounts.filter(a => a.account_type === 'ASSET').map(a => ({ ...normalizeAccount(a), balance: calcBalance(a) })).filter(a => a.balance !== 0);
    const liabilities = accounts.filter(a => a.account_type === 'LIABILITY').map(a => ({ ...normalizeAccount(a), balance: calcBalance(a) })).filter(a => a.balance !== 0);
    const equity      = accounts.filter(a => a.account_type === 'EQUITY').map(a => ({ ...normalizeAccount(a), balance: calcBalance(a) })).filter(a => a.balance !== 0);
...
router.get('/reports/cash-flow',
  ...
  asyncHandler(async (req, res) => {
    ...
    for (const line of cashLines) {
      ...
      const item = { description: desc, amount, date: line.journal_entries?.entry_date };
