...
interface EnabledModulesBody {
  p_module_codes?: string[];
}
...
interface HospitalMigrationBody {
  target_type?: string;
}
...
interface Rl6ReportBody {
  month?: number;
  year?: number;
}

interface QueueCallParams {
  id: string;
}

interface QueueDepartmentParams {
  departmentId: string;
}

// Helper: serialize any value to string for storage
function serializeSettingValue(val: unknown): string {
...
function deserializeSettingValue(str: string | null | undefined): unknown {
...
router.get('/admin/setup-status', asyncHandler(async (_req: Request, res: Response) => {
  const { prisma } = await import('../config/database.js');
  const setting = await prisma.system_settings.findUnique({
    where: { setting_key: 'setup_completed' }
  });
...
router.get('/admin/audit-logs', requireRole(['admin']), asyncHandler(async (req: Request<Record<string, string>, any, any, AuditLogQuery>, res: Response) => {
  const { prisma } = await import('../config/database.js');
...
router.get('/admin/modules', asyncHandler(async (req: Request<Record<string, string>, any, any, ModulesQuery>, res: Response) => {
  const { prisma } = await import('../config/database.js');
...
router.post('/admin/enabled-modules', requireRole(['admin']), asyncHandler(async (req: Request<Record<string, string>, any, EnabledModulesBody>, res: Response) => {
  const { prisma } = await import('../config/database.js');
...
router.post('/admin/enabled-modules/toggle', requireRole(['admin']), asyncHandler(async (req: Request<Record<string, string>, any, ToggleModuleBody>, res: Response) => {
  const { prisma } = await import('../config/database.js');
  ...
  const updated = p_enabled
    ...
    : current.filter((c: string) => c !== p_module_code);
...
router.get('/admin/menu-access', asyncHandler(async (req: Request, res: Response) => {
  const { prisma } = await import('../config/database.js');
...
router.post('/admin/reset-system', requireRole(['admin']), asyncHandler(async (_req: Request, res: Response) => {
  const { prisma } = await import('../config/database.js');
  await prisma.system_settings.upsert({
    where: { setting_key: 'setup_completed' },
...
router.post('/admin/hospital-migration/preview', requireRole(['admin']), asyncHandler(async (req: Request<Record<string, string>, any, HospitalMigrationBody>, res: Response) => {
  const { target_type } = req.body;
  res.json({ success: true, data: { target_type, modules_affected: [] } });
}));

// Execute hospital type migration
router.post('/admin/hospital-migration/execute', requireRole(['admin']), asyncHandler(async (req: Request<Record<string, string>, any, HospitalMigrationBody>, res: Response) => {
  const { prisma } = await import('../config/database.js');
  const { target_type } = req.body;
  await prisma.system_settings.upsert({
...
router.get('/reports/revenue', requireRole(['admin', 'keuangan', 'direktur']), asyncHandler(async (req: Request<Record<string, string>, any, any, RevenueQuery>, res: Response) => {
  const { prisma } = await import('../config/database.js');
...
router.get('/reports/visits', asyncHandler(async (req: Request<Record<string, string>, any, any, ReportQuery>, res: Response) => {
  const { prisma } = await import('../config/database.js');
  const { date_from, date_to } = req.query;
...
router.post('/reports/rl6-indicators', asyncHandler(async (req: Request<Record<string, string>, any, Rl6ReportBody>, res: Response) => {
  const { prisma } = await import('../config/database.js');
...
router.get('/queue/:departmentId', asyncHandler(async (req: Request<QueueDepartmentParams>, res: Response) => {
  const { prisma } = await import('../config/database.js');
...
router.post('/queue/:id/call', asyncHandler(async (req: Request<QueueCallParams>, res: Response) => {
  const { prisma } = await import('../config/database.js');
  const { id } = req.params;
