...
// Type definitions
interface PackageBody extends z.infer<typeof packageSchema> { }
 ...
 */
router.get('/packages', asyncHandler(async (req: Request<Record<string, string>, any, any, PackageQuery>, res: Response) => {
...
router.post('/packages',
  ...
  asyncHandler(async (req: Request<Record<string, string>, any, PackageBody>, res: Response) => {
...
router.post('/clients',
  requireRole([ROLES.ADMIN, ROLES.MANAJEMEN]),
  asyncHandler(async (req: Request<Record<string, string>, any, CorporateClientBody>, res: Response) => {
 ...
 */
router.get('/registrations', paginate, asyncHandler(async (req: Request<Record<string, string>, any, any, RegistrationQuery>, res: Response) => {
  ...
  res.json(paginatedResponse(registrations, total, req.pagination as any));
}));
...
router.post('/registrations',
  ...
  asyncHandler(async (req: Request<Record<string, string>, any, RegistrationBody>, res: Response) => {
    ...
    const registration = await prisma.$transaction(async (tx) => {
      ...
      let finalPrice = pkg.base_price.toNumber();
      if (corporateClientId) {
        const client = await tx.corporate_clients.findUnique({
          where: { id: corporateClientId }
        });
        if (client?.discount_percentage) {
          finalPrice = pkg.base_price.toNumber() * (1 - client.discount_percentage.toNumber() / 100);
        }
      }
...
router.patch('/registrations/:id/status',
  ...
  asyncHandler(async (req: Request<{ id: string }, any, UpdateStatusBody>, res: Response) => {
    const { id } = req.params;
 ...
 */
router.get('/registrations/:id/results', asyncHandler(async (req: Request<{ id: string }>, res: Response) => {
  const { id } = req.params;
...
router.post('/registrations/:id/results',
  ...
  asyncHandler(async (req: Request<{ id: string }, any, McuResultBody>, res: Response) => {
    const { id } = req.params;
...
router.get('/reports/summary',
  ...
  asyncHandler(async (req: Request<Record<string, string>, any, any, ReportQuery>, res: Response) => {
    ...
    res.json({
      ...
      data: {
        totalRegistrations,
        completedCount,
        totalRevenue: revenue._sum.total_price?.toNumber() || 0,
