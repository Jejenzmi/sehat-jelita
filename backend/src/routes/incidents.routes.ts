...
interface IncidentBody extends z.infer<typeof IncidentSchema> { }
interface IncidentUpdateBody extends z.infer<typeof IncidentUpdateSchema> { }
interface StatusBody extends z.infer<typeof StatusUpdateSchema> { }

async function generateIncidentCode() {
  try {
    const result = await prisma.$queryRaw`SELECT generate_incident_code() AS code` as Array<{ code: string }>;
...
// LIST
router.get('/', asyncHandler(async (req: Request<Record<string, string>, any, any, IncidentsQuery>, res: Response) => {
  ...
  if (from || to) {
    where.incident_date = {};
    if (from) (where.incident_date as Record<string, unknown>).gte = new Date(from);
    if (to) (where.incident_date as Record<string, unknown>).lte = new Date(to);
  }
  if (search) where.OR = [
...
// CREATE
router.post('/', asyncHandler(async (req: Request<Record<string, string>, any, IncidentBody>, res: Response) => {
...
// STATUS UPDATE
router.patch('/:id/status', asyncHandler(async (req: Request<{ id: string }>, res: Response) => {
