...
interface HospitalProfileUpdateBody {
  hospital_name?: string;
  address?: string;
}
 ...
 */
router.post('/', asyncHandler(async (req: Request<{}, {}, HospitalProfileBody>, res: Response) => {
  const {
    hospital_name,
    hospital_code,
    hospital_type,
  ...
  const hospitalInfoJson = JSON.stringify({
    name: hospital_name,
    code: hospital_code || '',
    address: address || '',
    city: city || '',
  ...
  const settingsToSave: [string, string][] = [
    ...
    ['hospital_info',        hospitalInfoJson], // This is what the frontend Settings page loads
  ];
 ...
 */
router.put('/:id', asyncHandler(async (req: Request<{ id: string }, {}, HospitalProfileUpdateBody>, res: Response) => {
  const { id } = req.params;
