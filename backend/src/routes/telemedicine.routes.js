...
router.get('/stats', asyncHandler(async (_req, res) => {
  ...
  const [todayTotal, waiting, completed, allCompleted] = await Promise.all([
    ...
    }).catch(() => ({ _avg: { duration_minutes: null } })),
  ]);
  ...
  res.json({
    ...
    data: {
      ...
      waiting,
      completed,
      avg_duration: allCompleted._avg?.duration_minutes
        ? Math.round(allCompleted._avg.duration_minutes)
        : 0,
    },
  });
}));
...
router.post('/sessions/:id/signal', asyncHandler(async (req, res) => {
...
router.get('/sessions/:id/signals', asyncHandler(async (req, res) => {
...
// Returns STUN/TURN credentials to authenticated frontend clients.
// TURN credentials use time-limited HMAC so they expire automatically.

router.get('/ice-servers', asyncHandler(async (_req, res) => {
  const iceServers = [];

  // Always include Google's public STUN (no credentials needed)
  iceServers.push({ urls: 'stun:stun.l.google.com:19302' });
  iceServers.push({ urls: 'stun:stun1.l.google.com:19302' });

  // Add configured TURN server if env vars are present
  ...
  if (turnHost && turnSecret) {
    // Time-Limited Credentials (RFC 7635): expire in 1 hour
