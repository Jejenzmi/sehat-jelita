...
const defaultMappings: Record<string, AnalyzerMapping> = {
...
// -- State Management --
let analyzerMappings: Record<string, AnalyzerMapping> = { ...defaultMappings };
let lisServer: net.Server | null = null;
...
export interface ProtocolStatus {
  astm: ConnectionStatus;
  hl7: ConnectionStatus;
}

const connectionStatus: ProtocolStatus = {
...
interface ASTMRecords {
  header: { sender: string; receiver: string; timestamp: string } | null;
  patient: { id: string; name: string; dob: string; gender: string } | null;
  orders: Array<{ order_id: string; sample_id: string; test_id: string; priority: string }>;
  results: Array<{
    test_id: string;
    value: string;
    unit: string;
...
function parseASTMMessage(frames: string[]): ASTMRecords {
  const records: ASTMRecords = { header: null, patient: null, orders: [], results: [] };
...
interface HL7Records {
  header: { sender: string; receiver: string; timestamp: string; message_type: string; control_id: string } | null;
  patient: { id: string; name: string; dob: string; gender: string } | null;
  orders: Array<{ order_id: string; sample_id: string; test_id: string; order_datetime: string }>;
  results: Array<{
    test_id: string;
    value: string;
    unit: string;
...
function parseHL7Message(rawMessage: string): HL7Records {
  const segments = rawMessage.split('\r').filter(s => s.length > 0);
  const records: HL7Records = { header: null, patient: null, orders: [], results: [] };
...
async function processAnalyzerResults(records: ASTMRecords | HL7Records, protocol: string = 'ASTM'): Promise<ProcessResult> {
  ...
  for (const result of records.results) {
    ...
    if (labOrder) {
      ...
      try {
        ...
        errors.push({ test_id: result.test_id, error: (e as Error).message });
      }
    } else {
...
export function startASTMServer(port: number = 9001): void {
  if (lisServer) return;
  ...
  lisServer = net.createServer((socket) => {
        ...
        if (byte === ASTM.ENQ) {
          ...
          if (frames.length > 0) {
            try {
              ...
              logger.error('ASTM processing error', { error: (e as Error).message });
            }
          }
          frames = [];
          buffer = '';
    ...
    socket.on('error', (err) => {
      logger.warn('ASTM socket error', { error: (err as Error).message });
    });
  });
  ...
  lisServer.on('error', (err) => {
    logger.error('ASTM server error', { error: (err as Error).message });
  });
}

export function startHL7Server(port: number = 9002): void {
  if (hl7Server) return;
  ...
  hl7Server = net.createServer((socket) => {
    ...
    socket.on('data', async (data: Buffer) => {
      ...
      while (true) {
        ...
        try {
          ...
          logger.error('HL7 processing error', { error: (e as Error).message });
        }
      }
    });
    ...
    socket.on('error', (err) => {
      logger.warn('HL7 socket error', { error: (err as Error).message });
    });
  });
...
function buildHL7ACK(header: { sender: string; receiver: string; timestamp: string; message_type: string; control_id: string } | null): string {
...
export function getStatus(): ProtocolStatus {
  return connectionStatus;
}

export function getMappings(): Record<string, AnalyzerMapping> {
  return analyzerMappings;
}

export function updateMapping(analyzerCode: string, mapping: AnalyzerMapping): void {
