...
function applyCellStyle(row: ExcelJS.Row): void {
  row.eachCell(cell => { Object.assign(cell, CELL_STYLE); });
}

function addHospitalHeader(sheet: ExcelJS.Worksheet, hospitalInfo: Record<string, string>): void {
...
async function getHospitalInfo(): Promise<HospitalInfo> {
  ...
  settings.forEach(s => { info[s.setting_key.replace('hospital_', '') as keyof HospitalInfo] = s.setting_value as string; });
  return info;
}

// -- Billing Export --

export interface ExportBillingOptions {
  date_from?: string;
  date_to?: string;
  format?: string;
}

export async function exportBilling({ date_from, date_to, format = 'xlsx' }: ExportBillingOptions): Promise<Buffer> {
  const where: Record<string, unknown> = { status: 'paid' };
  if (date_from || date_to) {
    where.payment_date = {};
    if (date_from) (where.payment_date as Record<string, unknown>).gte = new Date(date_from);
    if (date_to) (where.payment_date as Record<string, unknown>).lte = new Date(date_to);
  }
  ...
  billings.forEach((b, i) => {
    ...
    grandPaid += Number(b.paid_amount) || 0;
  });
...
export async function exportLabResult(orderId: string): Promise<Buffer> {
...
export interface ExportAccountingJournalOptions {
  date_from?: string;
  date_to?: string;
}

export async function exportAccountingJournal({ date_from, date_to }: ExportAccountingJournalOptions): Promise<Buffer> {
  const where: Record<string, unknown> = {};
  if (date_from || date_to) {
    where.entry_date = {};
    if (date_from) (where.entry_date as Record<string, unknown>).gte = new Date(date_from);
    if (date_to) (where.entry_date as Record<string, unknown>).lte = new Date(date_to);
  }
...
export interface ExportKemenkesRLOptions {
  report_type: string;
  month: number;
  year: number;
}

export async function exportKemenkesRL({ report_type, month, year }: ExportKemenkesRLOptions): Promise<Buffer> {
  const hospitalInfo = await getHospitalInfo();
  const workbook = new ExcelJS.Workbook();
