...
interface PaymentTypeReport {
  ...
  _sum: { paid_amount: number | string | null };
  _count: number;
}
...
interface MaintenanceDueResult {
  success: boolean;
...
export async function processReportJob(job: ReportJob): Promise<ReportResult> {
  const { name, data } = job;
  ...
  switch (name) {
    case 'daily-revenue': {
      ...
      const [revenue, byType, visitCount, newPatients] = await Promise.all([
        ...
        prisma.billings.groupBy({
          ...
          _sum: { paid_amount: true },
          _count: true
        }) as unknown as PaymentTypeReport[],
      ...
      await prisma.scheduled_reports.upsert({
        ...
        update: { status: 'completed', data: reportData as unknown as object, generated_at: new Date() },
        create: {
    ...
    case 'monthly-kpi': {
      ...
      const totalPatientDays = admissions.reduce((sum, a) => {
        ...
        const days = Math.max(1, Math.ceil((discharge.getTime() - new Date(a.admission_date).getTime()) / 86400000));
        return sum + days;
      }, 0);
      ...
      // TODO: Add discharge_status field to inpatient_admissions schema to re-enable death count
      const deaths = 0;
      // const deaths = await prisma.inpatient_admissions.count({
      //   where: { discharge_date: { gte: start, lte: end }, discharge_status: 'meninggal' }
      // }).catch(() => 0);

      const [revenue, visits, labs, prescriptions] = await Promise.all([
        prisma.billings.aggregate({
          where: { payment_date: { gte: start, lte: end }, status: 'paid' },
          _sum: { paid_amount: true }
        }),
      ...
      const NDR = admissions.length >= 48
        ? ((deaths / admissions.length) * 1000).toFixed(2)
        : 0;
      ...
      const reportData = {
        ...
        inpatient: {
          ...
          ALOS: parseFloat(ALOS as string),
          NDR: parseFloat(NDR as string),
        },
        total_revenue: Number(revenue._sum.paid_amount || 0),
        visits,
        lab_orders: labs,
      ...
      await prisma.analytics_snapshots.upsert({
        ...
        update: { data: reportData as unknown as object, computed_at: new Date() },
        create: {
          snapshot_type: 'monthly_kpi',
          snapshot_date: start,
          data: reportData as unknown as object
    ...
    case 'bed-occupancy-snapshot': {
      ...
      await prisma.analytics_snapshots.upsert({
        ...
        update: { data: { occupied, total, BOR } as unknown as object, computed_at: new Date() },
        create: {
          snapshot_type: 'daily_bed_occupancy',
          snapshot_date: date,
          data: { occupied, total, BOR } as unknown as object
        }
      }).catch(() => { });
      ...
      for (const row of rows || []) {
        try {
          const existing = row.nik
            ? await prisma.patients.findFirst({ where: { nik: row.nik as string } })
            : null;
          if (existing) {
            failed++;
          ...
          await prisma.patients.create({
            data: { ...row, medical_record_number } as any
          });
          imported++;
        } catch (err) {
          failed++;
    ...
    case 'db-backup': {
      ...
      try {
        ...
        console.error('[Backup] pg_dump failed:', (err as Error).message);
        // Log failure
        await prisma.notification_logs.create({
          data: {
            channel_type: 'system',
            ...
            payload: { error: (err as Error).message, timestamp },
            status: 'failed',
    ...
    case 'check-cert-expiry': {
      ...
      const certsToAlert = await prisma.staff_certifications.findMany({
        ...
        include: {
          employees: {
            select: { full_name: true, employee_code: true, departments: { select: { department_name: true } } },
          },
        },
      });
      ...
      for (const cert of certsToAlert) {
        const daysLeft = Math.ceil((new Date(cert.expiry_date).getTime() - now.getTime()) / 86400000);
        const employeeName = (cert as unknown as { employees?: { full_name?: string } }).employees?.full_name || cert.user_id || 'Unknown';
        const deptName = (cert as unknown as { employees?: { departments?: { department_name?: string } } }).employees?.departments?.department_name || '';
        ...
        if (!(cert as unknown as { alert_sent_30d?: boolean }).alert_sent_30d && daysLeft <= 30) {
          await prisma.notification_logs.create({
            data: {
              channel_type: 'system',
        ...
        if (!(cert as unknown as { alert_sent_7d?: boolean }).alert_sent_7d && daysLeft <= 7) {
          await prisma.notification_logs.create({
            data: {
              channel_type: 'system',
              recipient: employeeName,
          ...
          data: {
            ...
            payload: {
              ...
              assets: assetsDue.map(a => ({
                code: a.asset_code,
                name: a.asset_name,
                dept: (a as unknown as { departments?: { name?: string } }).departments?.name,
