...
export function LobbyDisplay() {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 flex-1 min-h-0 mt-3">
        {/* Main Queue */}
        <div className="lg:col-span-2 flex flex-col gap-4 min-h-0">
          {/* Currently Called */}
          <Card className="border-2 border-teal-200 dark:border-teal-800 shadow-lg overflow-hidden">
            <CardHeader className="pb-2 bg-gradient-to-r from-teal-50 to-cyan-50 dark:from-teal-950/40 dark:to-cyan-950/40">
            ...
            <CardContent className="pt-3 pb-3">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {called.map((q: any) => (
                  <div key={q.id} className="flex items-center gap-4 p-3 bg-gradient-to-r from-teal-500 to-cyan-500 rounded-xl text-white shadow-md animate-fade-in">
                    <div className="text-3xl font-black tracking-wider">{q.ticket_number}</div>
                    <div className="flex-1 min-w-0">
                      <p className="font-bold truncate">{q.patients?.full_name || q.patient_name || "-"}</p>
                ...
                {called.length === 0 && (
                  <p className="text-muted-foreground col-span-2 text-center py-6">Belum ada antrian dipanggil</p>
                )}
              </div>
            </CardContent>
          </Card>
          ...
          {/* Waiting Queue */}
          <Card className="shadow-md flex-1 min-h-0 flex flex-col">
            ...
            <CardContent className="flex-1 min-h-0 overflow-hidden pb-3">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                {waiting.slice(0, 8).map((q: any, i: number) => (
                  <div key={q.id} className={`flex items-center gap-3 p-2 rounded-xl transition-colors ${i < 3 ? "bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800" : "bg-muted/50"}`}>
                    <div className={`text-lg font-bold w-16 ${i < 3 ? "text-amber-600" : "text-muted-foreground"}`}>{q.ticket_number}</div>
                    <div className="flex-1 min-w-0">
                ...
                {waiting.length === 0 && (
                  <p className="text-muted-foreground text-center py-4 col-span-2">Belum ada antrian menunggu</p>
                )}
              </div>
            </CardContent>
          </Card>

          {/* Image Slideshow - fixed height */}
          <div className="flex-shrink-0 h-48 rounded-xl overflow-hidden">
            <ImageSlideshow />
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4 min-h-0">
          ...
          <Card className="bg-muted/50 border-dashed flex-shrink-0">
            <CardContent className="p-3 space-y-1.5">
              <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-1">Hubungi Kami</p>
